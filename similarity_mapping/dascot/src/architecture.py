...
def insert_column_left(arch):
    ...
    for m in arch["alg_qubits"]:
        # need to count first row
        row = m // arch["width"] + 1
        new["magic_states"].append(q + row)  # type: ignore
    return new
...
def all_sides(width, height):
    ...
    all_slots = list(
        dict.fromkeys(top_row + right_column + list(reversed(bottom_row)) + left_column)
    )
    msf = []
...
def square_sparse_layout(alg_qubit_count, magic_states):
    ...
    if magic_states == "all_sides":
        arch = insert_row_below(
            insert_row_above(insert_column_right(insert_column_left(arch)))
        )
